...
## calculate summary statistics for diversity indices including mean, sd, min, max
div_summary <- div_data2 %>%
    summarise(
        ...
        SD_InvSimpson = sd(InvSimpson, na.rm = TRUE),
        ...
        Mean_Eveness = mean(Eveness, na.rm = TRUE),
        SD_Evennss = sd(Eveness, na.rm = TRUE),
        Min_Eveness = min(Eveness, na.rm = TRUE),
        Max_Eveness = max(Eveness, na.rm = TRUE)
    )

## plot histograms for diversity indices from div_data2 and add summary statistics to the plots
plot_diversity_histogram <- function(data, index_name, summary_stats) {
    p <- ggplot(na.omit(data), aes_string(x = index_name)) +
        geom_histogram(binwidth = 0.1, fill = "grey", color = "black", alpha = 0.7) +
        labs(
            title = paste("Histogram of", index_name),
            x = index_name, y = "Frequency"
        ) +
        theme_bw() +
        geom_vline(aes(xintercept = summary_stats[[paste0("Mean_", index_name)]]), color = "red", linetype = "dashed") +
        geom_text(
            aes(
                x = summary_stats[[paste0("Mean_", index_name)]] + 0.1,
                y = 5,
                label = paste("Mean:", round(summary_stats[[paste0("Mean_", index_name)]], 2))
            ),
            color = "red", size = 4, vjust = -1
        ) +
        geom_text(
            aes(
                x = summary_stats[[paste0("Min_", index_name)]] - 0.1,
                y = 5,
                label = paste("Min:", round(summary_stats[[paste0("Min_", index_name)]], 2))
            ),
            color = "blue", size = 4, vjust = -1
        ) +
        geom_text(
            aes(
                x = summary_stats[[paste0("Max_", index_name)]] + 0.1,
                y = 5,
                label = paste("Max:", round(summary_stats[[paste0("Max_", index_name)]], 2))
            ),
            color = "green", size = 4, vjust = -1
        )
    return(p)
}
# Create histograms for each diversity index
histograms <- list(
    plot_diversity_histogram(div_data2, "Shannon", div_summary),
    plot_diversity_histogram(div_data2, "Simpson", div_summary),
    plot_diversity_histogram(div_data2, "Richness", div_summary),
    plot_diversity_histogram(div_data2, "Eveness", div_summary)
)
